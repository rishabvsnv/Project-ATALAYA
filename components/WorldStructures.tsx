...
function CampfireMesh({ struct }: { struct: WorldStructure }) {
  ...
  useFrame(({ clock }) => {
    if (lightRef.current) {
      ...
      lightRef.current.intensity = 1.4 + Math.sin(clock.elapsedTime * 12) * 0.35;
    }
  });
  ...
  return (
    <group position={struct.position} rotation={[0, struct.rotationY, 0]}>
      ...
      <mesh position={[0, 0.22, 0]}>
        ...
        <meshStandardMaterial color="#F97316" emissive="#EA580C" emissiveIntensity={1.2} flatShading />
      </mesh>
...
function ShelterMesh({ struct }: { struct: WorldStructure }) {
  return (
    <group position={struct.position} rotation={[0, struct.rotationY, 0]}>
      ...
      {/* Left Lean Wall */}
      <mesh position={[-0.45, 0.45, 0]} rotation={[0, 0, -0.45]} castShadow receiveShadow>

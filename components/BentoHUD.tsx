...
export function BentoHUD() {
  ...
  return (
    <aside aria-label="HUD Overlay" className="pointer-events-none fixed inset-0 z-20 flex flex-col justify-between p-6 select-none">
        ...
        <section aria-label="Survivor Vitals" className="pointer-events-auto w-84 rounded-2xl border border-white/10 bg-black/50 p-4 shadow-2xl backdrop-blur-md transition-all">
          ...
          <div className="space-y-2.5">
            ...
            <VitalBar
              label="Hunger"
              val={vitals.hunger}
              icon={<Utensils className="h-3.5 w-3.5 text-amber-400" />}
        ...
        <section aria-label="Environment and Time" className="pointer-events-auto flex items-center gap-4 rounded-2xl border border-white/10 bg-black/50 px-5 py-3.5 shadow-2xl backdrop-blur-md">
          <div className="flex items-center gap-3">
            <div className="relative flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/5 overflow-hidden">
              ...
              >
                <Sun className="absolute top-1 h-3.5 w-3.5 text-amber-400" />
                <div className="absolute bottom-1 h-2 w-2 rounded-full bg-blue-300 shadow-[0_0_6px_#93c5fd]" />
              </div>
            </div>
            <div>
              <p className="text-xs font-medium tracking-wide text-neutral-400 uppercase">Celestial Epoch</p>
              <p className="text-sm font-semibold text-neutral-100">Day {day} • {timeOfDay}</p>
            </div>
          </div>
      ...
      {/* Bottom Area: Controls Bar + Grid Monologue / Inventory */}
        ...
        <div className="flex justify-center">
          <nav aria-label="Simulation Controls" className="pointer-events-auto flex items-center gap-3 rounded-full border border-white/15 bg-black/70 px-4 py-2 shadow-2xl backdrop-blur-xl transition-all">
            ...
            <button
              ...
              className="flex h-8 w-8 items-center justify-center rounded-full bg-white/5 text-neutral-300 hover:bg-white/10 disabled:opacity-30 disabled:pointer-events-none transition-all"
            ...
            <div className="flex items-center gap-1">
              ...
              {speedOptions.map((speed) => (
                <button
                  ...
                  className={`rounded-full px-2.5 py-0.5 text-xs font-mono font-semibold transition-all ${
                    simSpeed === speed
  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between text-xs">
        ...
        <span className="font-mono text-xs text-neutral-400">{Math.round(val)}%</span>
      </div>
